    ...
    async def fetch_recent_news(self, *, limit: int = 30) -> list[dict[str, Any]]:
        try:
            async with httpx.AsyncClient(
                ...
                timeout=httpx.Timeout(8.0, connect=5.0, read=6.0),
                follow_redirects=True,
                headers={
                    "User-Agent": "momo-trading/1.0 (+https://localhost:9000/admin)",
        ...
        except (httpx.TimeoutException, httpx.HTTPError, ElementTree.ParseError) as exc:
            raise RuntimeError("Nasdaq 소스 응답 지연 또는 비정상 연결") from exc
