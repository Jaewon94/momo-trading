...
@pytest.mark.asyncio
async def test_app_lifespan_resets_llm_runtime_after_loading_persisted_settings(monkeypatch) -> None:
    recorder = Recorder()

    async def fake_apply_persisted_settings() -> dict:
        recorder.calls.append("settings.apply")
        return {"CODEX_MODEL_TIER1": "gpt-5.4"}
    ...
    monkeypatch.setattr("main.setup_logging", lambda: None)
    monkeypatch.setattr("main.runtime_settings_service.apply_persisted_settings", fake_apply_persisted_settings)
    monkeypatch.setattr(
        "analysis.llm.llm_factory.llm_factory.reset_runtime_state",
        lambda: recorder.calls.append("llm.reset"),
    )
    monkeypatch.setattr("main.event_bus.start", fake_event_bus_start)
    monkeypatch.setattr("main.event_bus.stop", fake_event_bus_stop)
    monkeypatch.setattr("main.broker_runtime_service.startup", fake_runtime_startup)
    ...
    assert recorder.calls.index("settings.apply") < recorder.calls.index("llm.reset")
