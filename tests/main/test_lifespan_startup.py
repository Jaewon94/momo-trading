...
async def test_app_lifespan_starts_even_when_mcp_connect_fails(monkeypatch) -> None:
    ...
    async def fake_mcp_connect() -> None:
        recorder.calls.append("mcp.connect")
        raise RuntimeError("mcp down")

    async def fake_mcp_disconnect() -> None:
        recorder.calls.append("mcp.disconnect")
    ...
    assert recorder.calls[:4] == ["setup_logging", "validate", "event_bus.start", "mcp.connect"]
    assert recorder.calls.count("start") == 3
    assert recorder.calls.count("stop") == 3
    assert "inside" in recorder.calls
    assert "mcp.disconnect" in recorder.calls
