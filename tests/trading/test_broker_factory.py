...
def test_build_broker_adapter_returns_kiwoom_adapter() -> None:
    adapter = build_broker_adapter(BrokerProvider.KIWOOM)
...
def test_build_broker_adapter_rejects_unsupported_provider() -> None:
    with pytest.raises(ValueError):
        build_broker_adapter("UNSUPPORTED")  # type: ignore[arg-type]
