        ...
        [
            {
                ...
                "max_hold_days": 15,
                "trade_horizon": "MID",
                "min_hold_minutes_before_review_exit": 1440,
                "min_hold_minutes_before_profit_exit": 1440,
                "min_hold_minutes_before_soft_stop_exit": 1440,
                "confidence": 0.82,
                "target_price": 75_000,
...
def test_holdings_review_prompt_exposes_horizon_contract() -> None:
    ...
    assert "리뷰매도 1440분" in prompt
    assert "수익/부분익절 1440분" in prompt
    assert "소프트손절 1440분" in prompt
    ...
    assert "MID는 며칠~수주 스윙" in HOLDINGS_REVIEW_SYSTEM
