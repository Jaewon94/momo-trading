...
def test_llm_runtime_recommendation_service_keeps_news_model_when_translation_disabled(monkeypatch):
    monkeypatch.setattr("services.llm_runtime_recommendation_service.settings.NEWS_LLM_ENABLED", True)
    monkeypatch.setattr("services.llm_runtime_recommendation_service.settings.NEWS_TRANSLATE_FOREIGN_ENABLED", False)
    monkeypatch.setattr("analysis.llm.selection_policy.settings.NEWS_LLM_PROVIDER", "OLLAMA")
    monkeypatch.setattr("analysis.llm.selection_policy.settings.NEWS_LLM_MODEL", "qwen3:14b")
    monkeypatch.setattr("analysis.llm.selection_policy.settings.NEWS_LLM_FALLBACK_PROVIDER", "")
    monkeypatch.setattr("analysis.llm.selection_policy.settings.MANUAL_LLM_PROVIDER", "CODEX")
    ...
    assert payload["news_translation"]["current"]["model"] == "qwen3:14b"
    assert payload["news_translation"]["recommended"]["model"] == "qwen3:14b"
    assert payload["news_translation"]["action"] == "KEEP"
    assert "비활성화" in payload["news_translation"]["reasons"][0]
