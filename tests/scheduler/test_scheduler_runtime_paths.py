...
@pytest.mark.asyncio
async def test_holdings_check_blocks_profit_guard_stop_before_mid_min_hold(monkeypatch) -> None:
    scheduler = TradingScheduler()
    logs: list[str] = []
    observed_at = datetime(2026, 6, 8, 10, 21)
    holding = SimpleNamespace(symbol="459550", name="알트", quantity=1800, avg_buy_price=2_035)
    trade_result = SimpleNamespace(
        stock_symbol="459550",
        strategy_type="STABLE_SHORT",
        ai_stop_loss_price=2_037,
        ai_target_price=2_250,
        notes='{"trade_horizon":"MID","active_stop_loss":1972}',
        entry_at=observed_at - timedelta(minutes=25),
    )
    ...
    async def fake_place_market_sell(*_args, **_kwargs):
        pytest.fail("profit-guard stop before MID min hold must not sell")
    ...
    monkeypatch.setattr(
        "realtime.event_detector.event_detector.get_thresholds",
        lambda _symbol: SimpleNamespace(stop_loss=2_037, take_profit=2_250, trailing_stop_pct=0.0),
    )
    monkeypatch.setattr("scheduler.scheduler.settings.TRADING_ENABLED", True)
