...
class DecisionMaker:
    ...
    async def wait_for_inflight_confirms(self, timeout_sec: float) -> bool:
        """진행 중인 confirm_and_record 태스크가 끝날 때까지 비파괴적으로 대기.

        타임아웃이 발생해도 태스크를 취소하지 않고 False만 반환한다 (체결 확인이
        외부 reconfig에 의해 끊기지 않도록).
        """
    ...
    async def _execute_autonomous(
        ...
        if signal.action == OrderSide.BUY:
            pending_block = await self._find_pending_buy_block(signal.symbol)
            if pending_block:
                message = (
                    "미확정 매수 주문 확인 대기 중: "
                    f"{pending_block.stock_name or pending_block.stock_symbol}"
                    f"({pending_block.stock_symbol}) 주문번호 {pending_block.order_id or '-'}"
                )
                await activity_logger.log(
                    ActivityType.DECISION,
                    ActivityPhase.SKIP,
                    f"⏸️ [{signal.symbol}] 주문 보류: {message}",
                    cycle_id=cycle_id,
                    symbol=signal.symbol,
                )
                result = {
        ...
        if is_submitted:
            ...
            # 자동 주문에서는 확인 작업을 같은 흐름에서 끝낸다.
            # 백그라운드로 넘기면 브로커/DB 불일치 시 PENDING_CONFIRM이 오래 남아
            # 다음 주문 판단을 막거나 중복 주문 위험을 키울 수 있다.
            await self.confirm_and_record(
                symbol=signal.symbol,
                side=signal.action.value,
                order_id=order_id,
                quantity=qty,
    ...
    async def _find_pending_buy_block(self, symbol: str) -> TradeResult | None:
    ...
    def _build_trade_notes(analysis_context: dict | None, *, pending: bool = False) -> str | None:
        ...
        payload = {
            ...
            "alpha_source": ctx.get("alpha_source"),
    ...
    def _calculate_hold_days(closed_at, entry_at) -> int:
        if not entry_at:
            return 0
        comparable_closed_at = closed_at
        if (
            getattr(entry_at, "tzinfo", None) is None
            and getattr(closed_at, "tzinfo", None) is not None
        ):
            comparable_closed_at = closed_at.replace(tzinfo=None)
        elif (
            getattr(entry_at, "tzinfo", None) is not None
            and getattr(closed_at, "tzinfo", None) is None
        ):
            entry_at = entry_at.replace(tzinfo=None)
        return max((comparable_closed_at - entry_at).days, 0)

    @classmethod
    def _neutral_close_excess_open_buys(
        cls,
        session,
        ...
        *,
        ...
        for open_buy in sorted_buys:
            if remaining_to_close <= 0:
                break
            if getattr(open_buy, "exit_at", None) is not None:
                continue
            ...
            entry_price = float(getattr(open_buy, "entry_price", 0.0) or 0.0)
            target.exit_price = entry_price
            target.pnl = 0.0
            target.return_pct = 0.0
            target.is_win = False
            target.hold_days = cls._calculate_hold_days(closed_at, getattr(open_buy, "entry_at", None))
            target.exit_reason = "BROKER_HOLDING_QUANTITY_MISMATCH"
            target.exit_at = closed_at
            previous_notes = str(getattr(target, "notes", "") or "").strip()
            note = (
                "HOLDING_RECONCILIATION_CLOSE: broker holding quantity lower; "
                f"{note_reason} | target_open_qty={target_open_qty}, db_qty={current_open_qty}, close_qty={close_qty}"
            )
            target.notes = f"{note} | previous={previous_notes[:160]}" if previous_notes else note
    ...
    async def confirm_and_record(
        ...
        try:
            ...
            confirm_timeout_sec = float(
                getattr(settings, "ORDER_CONFIRM_STATUS_TIMEOUT_SEC", 15) or 15
            )
            try:
            ...
            if not order_status:
                ...
                await self._mark_pending_failed(
                    pending_record_id, "체결내역에서 미발견 (브로커 응답 없음)"
                )
                if on_settled:
                    await on_settled(order_id, False)
                return False
            ...
            if filled_qty <= 0:
                ...
                await self._cancel_unfilled_order(order_id, symbol)
                await self._mark_pending_failed(
                    pending_record_id, "체결수량 0 (주문 취소됨)"
                )
                if on_settled:
                    await on_settled(order_id, False)
                return False
            ...
            logger.warning(
                "[{}] 체결 확인 취소 (주문번호 {}) — PENDING_CONFIRM 유지, portfolio_sync에서 복구 예정",
                symbol, order_id,
            )
            raise
        except Exception as e:
    ...
    def _order_confirm_wait_sec(side: str) -> float:
        if str(side or "").upper() != "BUY":
            return float(getattr(settings, "SELL_ORDER_CONFIRM_WAIT_SEC", 3) or 3)

        risk_appetite = str(getattr(settings, "RISK_APPETITE", "MODERATE") or "MODERATE").upper()
        setting_name_by_risk = {
            "CONSERVATIVE": "BUY_ORDER_CONFIRM_WAIT_SEC_CONSERVATIVE",
            "MODERATE": "BUY_ORDER_CONFIRM_WAIT_SEC_MODERATE",
            "AGGRESSIVE": "BUY_ORDER_CONFIRM_WAIT_SEC_AGGRESSIVE",
        }
        setting_name = setting_name_by_risk.get(
            risk_appetite,
        ...
        try:
            ...
            holding = next(
                (
                    ...
                    if normalize_krx_symbol(getattr(item, "symbol", "")) == normalized_symbol
                ),
                None,
            )
            ...
            async with AsyncSessionLocal() as session:
                repo = TradeResultRepository(session)
                open_buys = await repo.get_all_open_buys(normalized_symbol)
                db_open_qty = sum(int(getattr(tr, "quantity", 0) or 0) for tr in open_buys)

            if db_open_qty <= 0 or current_qty >= db_open_qty:
                return False

            inferred_qty = db_open_qty - current_qty
            requested_qty = int(requested_quantity or 0)
            if requested_qty > 0:
                inferred_qty = min(inferred_qty, requested_qty)
            if inferred_qty <= 0:
                return False

            fill_price = float(expected_price or 0.0)
            if fill_price <= 0:
                fill_price = float(getattr(holding, "current_price", 0.0) or 0.0)
            if fill_price <= 0:
                logger.warning(
                    "[{}] SELL 체결 추론 보류: 주문번호={} — 추정 체결가 없음",
                    normalized_symbol,
                    order_id,
                )
                return False

            inferred_context = dict(analysis_context or {})
            inferred_context["order_status_inferred_from_holdings"] = True
            await self._record_trade_result(
                ...
                analysis_context=inferred_context,
                exit_reason=exit_reason or "SIGNAL",
                cycle_id=cycle_id,
            )
            logger.warning(
                "[{}] SELL 체결 상태 미조회 → 보유수량 감소로 체결 추론: {}주 @{:,.0f}원 (주문번호: {})",
                normalized_symbol,
                inferred_qty,
                fill_price,
                order_id,
            ...
            await activity_logger.log(
                ...
                f"🧩 [{normalized_symbol}] SELL 체결 상태 미조회 → 보유수량 기준 청산 기록: "
                f"{inferred_qty}주 @{fill_price:,.0f}원",
                cycle_id=cycle_id,
                symbol=normalized_symbol,
                detail={
                    "order_id": order_id,
                    "requested_quantity": requested_qty,
                    "db_open_quantity": db_open_qty,
                    "broker_current_quantity": current_qty,
                    "inferred_quantity": inferred_qty,
            ...
            logger.warning("[{}] SELL 체결 보유수량 추론 실패: {}", symbol, str(exc))
            return False
    ...
    async def _adjust_sell_filled_qty_from_holdings(
        self,
        *,
        symbol: str,
        requested_quantity: int,
        reported_filled_qty: int,
        open_buys: list[TradeResult],
    ) -> int:
        """Kiwoom status can report a stale partial fill after a market SELL.

        If broker holdings already shrank more than the reported fill, prefer the
        holding delta so DB lots do not remain open after a real full exit.
        """
        normalized_symbol = normalize_krx_symbol(symbol)
        db_open_qty = sum(
            int(getattr(open_buy, "quantity", 0) or 0)
        ...
        try:
            ...
            for holding in holdings or []:
                if normalize_krx_symbol(getattr(holding, "symbol", "")) == normalized_symbol:
                    current_holding_qty = int(getattr(holding, "quantity", 0) or 0)
                    break

            inferred_filled_qty = max(db_open_qty - current_holding_qty, 0)
            requested_qty = int(requested_quantity or 0)
            if requested_qty > 0:
                inferred_filled_qty = min(inferred_filled_qty, requested_qty)

            reported_qty = int(reported_filled_qty or 0)
            if inferred_filled_qty > reported_qty:
                logger.warning(
                    "[{}] SELL 체결수량 보정: 브로커 상태 {}주 < DB {}주, reported={}주 → applied={}주",
                    normalized_symbol,
                    current_holding_qty,
                    db_open_qty,
                    reported_qty,
                ...
                return inferred_filled_qty
        except Exception as exc:
            logger.warning("[{}] SELL 체결수량 보유수량 보정 실패: {}", normalized_symbol, str(exc))
    ...
    async def _confirm_pending_record(
        ...
        now = now_kst()
        fallback_record_needed = False
        activity_log: dict | None = None
        try:
            async with AsyncSessionLocal() as session:
                async with session.begin():
                    repo = TradeResultRepository(session)
                    ...
                    if not tr:
                        ...
                        if side == "BUY":
                            ...
                            filled_qty = await self._adjust_sell_filled_qty_from_holdings(
                                symbol=symbol,
                                requested_quantity=int(getattr(tr, "quantity", 0) or filled_qty or 0),
                                reported_filled_qty=filled_qty,
                                open_buys=open_buys,
                            )
                            total_open_qty = sum(
                                int(getattr(open_buy, "quantity", 0) or 0)
                            ...
                            if filled_qty > 0 and total_open_qty > filled_qty:
                                try:
                                    holdings = await self._broker_adapter.get_holdings()
                                    current_holding_qty = 0
                                    for holding in holdings:
                                        if normalize_krx_symbol(getattr(holding, "symbol", "")) == symbol:
                                            current_holding_qty = int(getattr(holding, "quantity", 0) or 0)
                                            break
                                    if total_open_qty > current_holding_qty + filled_qty:
                                        neutral_fill = self._neutral_close_excess_open_buys(
                                            session,
                                            open_buys,
                                            symbol=symbol,
                                            target_open_qty=current_holding_qty + filled_qty,
                                            closed_at=now,
                                            note_reason="before sell confirmation",
                                        )
                                        if neutral_fill["closed_quantity"] > 0:
                                            logger.warning(
                                                "[{}] 매도 체결 전 DB 초과수량 중립 정리: {}주 ({}건)",
                                                symbol,
                                                neutral_fill["closed_quantity"],
                                                neutral_fill["closed_count"],
                                            )
                                except Exception as reconcile_exc:
                                    logger.warning("[{}] 매도 전 초과수량 정리 확인 실패: {}", symbol, str(reconcile_exc))
                        ...
                        activity_log = {
                            "activity_type": ActivityType.TRADE_RESULT,
                            "phase": ActivityPhase.COMPLETE,
                            "summary": f"\U0001f4dd [{symbol}] {side} 체결 확인: {filled_qty}주 @{filled_price:,.0f}원",
                            "cycle_id": cycle_id,
                            "symbol": symbol,
                        }
            if fallback_record_needed:
                await self._record_trade_result(
    ...
    async def _record_trade_result(
        ...
        try:
            async def _write_trade_result() -> None:
                attempt_activity_logs: list[dict] = []
                async with AsyncSessionLocal() as session:
                    async with session.begin():
                        repo = TradeResultRepository(session)
                        ...
                        if side == "BUY":
                            ...
                            adjusted_filled_qty = await self._adjust_sell_filled_qty_from_holdings(
                                symbol=symbol,
                                requested_quantity=filled_qty,
                                reported_filled_qty=filled_qty,
                                open_buys=open_buys,
                            ...
                            sell_fill = self._apply_sell_fill_to_open_buys(
                                ...
                                open_buys,
                                symbol=symbol,
                                filled_qty=adjusted_filled_qty,
                                filled_price=filled_price,
                                exit_reason=exit_reason,
                                closed_at=now,
                            )
                ...
                activity_logs[:] = attempt_activity_logs

            await run_sqlite_write_with_retry(_write_trade_result)
            for log_payload in activity_logs:
                await activity_logger.log(**log_payload)
