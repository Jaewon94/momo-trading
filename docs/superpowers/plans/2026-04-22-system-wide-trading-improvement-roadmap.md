...
- [x] **Step 3: 최소 구현**
  ...
  - 2026-04-27 추가 보강: latest account snapshot freshness를 `FRESH|OFF_SESSION_STALE|STALE|MISSING`으로 분류해 performance summary와 account balance payload에 노출한다. 자동매매 비지원 세션의 오래된 snapshot은 운영 대기 상태로 표시하고, 자동매매 가능 세션의 오래된 snapshot은 신규 BUY 차단 근거로 사용한다.
...
- [x] **Step 3: 최소 구현**
  ...
  - 2026-04-27 추가 보강: `snapshot_stale_blocks_buy=true`인 account snapshot 상태에서는 drawdown 수치가 양호해도 신규 BUY를 보류한다. NXT 애프터마켓처럼 자동매매 비지원 세션은 `OFF_SESSION_STALE`로 분리해 장애처럼 표시하지 않는다.
