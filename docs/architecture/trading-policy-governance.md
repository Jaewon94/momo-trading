...
| 56 | `news_intel` | CANDIDATE | News polling, enrichment, horizon policy, after-hours research guardrails, and prompt context evidence. | `NEWS_POLL_ENABLED`, `NEWS_LOOKBACK_HOURS`, `HORIZON_SHORT_NEWS_LOOKBACK_HOURS`, `HORIZON_MID_NEWS_LOOKBACK_HOURS`, `HORIZON_LONG_NEWS_LOOKBACK_HOURS` | `tests/strategy/test_news_intelligence_policy.py`, `tests/services/test_news_signal_service.py`, `tests/services/test_news_context_service.py` |
