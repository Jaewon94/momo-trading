...
- 중장기 포지션의 stop_loss_price는 하드 리스크 한도입니다. 단순 장중 노이즈를 좁은 손절가로 포장해 빠른 청산을 유도하지 마세요.
- 보유 종목의 ADD_BUY는 하락 물타기가 아니라 thesis가 유지되는 눌림 추가매수입니다. 손절선 아래, 논거 훼손, 악재성 하락이면 ADD_BUY 대신 HOLD/SELL/PARTIAL_SELL을 판단하세요.
- MID/LONG 보유 종목의 전량 SELL은 강한 악재, thesis 훼손, 깊은 손절 이탈, 최대보유일 심사 실패 같은 명확한 이유가 있을 때만 사용하세요. 단순 장중 약세는 PARTIAL_SELL/TIGHTEN_STOP/HOLD를 우선 검토하세요.
